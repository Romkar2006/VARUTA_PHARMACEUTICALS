...
export default function Globe({
    speed = 2,
    smoothing = 8,
    dots = { color: "#0b835c", size: 5, density: 8, allDots: false },
    fill = "dots",
    fillColor = "#0b835c",
    scale = 8,
    stopOnHover = true,
    markerConfig = { markers: [{ lat: 17.3850, lng: 78.4867 }, { lat: 18.5204, lng: 73.8567 }], color: "#0b835c", size: 40 },
    direction = "left",
    initialLatitude = 20,
    initialLongitude = 78,
    ...
    useEffect(() => {
        ...
        const cameraDistance = 2.95 / scaleMultiplier;
        ...
        const oceanColorObj = resolvedOceanColor && resolvedOceanColor !== "transparent"
            ? new Color(resolvedOceanColor)
            : new Color(1, 1, 1);
        const oceanMaterial = new MeshBasicMaterial({
            color: oceanColorObj,
            transparent: true,
            opacity: resolvedOceanColor === "transparent" ? 0 : oceanRgba.a,
        });
            ...
            try {
                ...
                if (fill === "solid") {
                    ...
                    if (dotCoordinates.length > 0) {
                        ...
                        const cGreen = new Color("#0b835c");
                        const cBlue = new Color("#0284c7");
                        const cCyan = new Color("#06b6d4");
                        ...
                        for (let i = 0; i < dotCoordinates.length; i++) {
                            ...
                            if (i % 3 === 0) {
                                instanced.setColorAt(i, cCyan);
                            } else if (i % 2 === 0) {
                                instanced.setColorAt(i, cBlue);
                            } else {
                                instanced.setColorAt(i, cGreen);
                            }
                        }
